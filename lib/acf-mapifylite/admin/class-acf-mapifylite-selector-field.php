...
if( ! class_exists( 'Acf_Mapifylite_Selector_Field' ) ) :


class Acf_Mapifylite_Selector_Field extends acf_field {
	...
	function __construct( $settings ) {
		...
		$this->label = __( 'Mapify Map Selector', 'acf-mapifylite' );
		 ...
		 * var message = acf._e('mapify_map_selector', 'error');
		 */
	...
	function render_field_settings( $field ) {
		...
		// Map Selector
		acf_render_field_setting( $field, array(
			'label'        => __( 'Map Selector', 'acf-mapifylite' ),
			'instructions' => __( 'The preset map locations that you can select later on the field', 'acf-mapifylite' )  . '<br /><br />' . __( 'For more control, you may specify both a value and label like this:', 'acf-mapifylite' ). '<br /><br />Jakarta : -6.1753942 : 106.827183',
			'type'         => 'textarea',
			'name'         => 'map_selector',
		));
	...
	function render_field( $field ) {
		...
		if ( isset( $value['post_id'] ) ) {
			$mapify_map  = new Mapify_Map( $value['post_id'] );
			$selector    = $mapify_map->get_map_locations();
			$selected    = $mapify_map->get_map_location_id();
		 ...
		 * Load admin fields
		 */
		include_once( 'partials/mapify-map-selector-field-display.php' );
	...
	function input_admin_enqueue_scripts() {
		...
		// register & include JS
		wp_register_script( 'acf-mapifylite-selector', "{$url}admin/js/script-map.js", array( 'jquery', 'acf-input', 'class-openstreetmap' ), $version );
		wp_enqueue_script( 'acf-mapifylite-selector' );
		wp_localize_script( 'acf-mapifylite-selector', 'osm_var', array(
	...
	/**
	 * Decode location choices
	 * from formatted string to array
	 */
	function decode_location_choices( $string = '' ) {
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		 ...
		 */
		if ( isset( $value['selected_dropdown'] ) && isset( $value['centered_lat'] ) ) {			
			$mapify_map = new Mapify_Map( $post_id );
			$mapify_map->set_map_location_id( intval( $value['selected_dropdown'] ) );
			$mapify_map->set_map_location_details( 
				$value['centered_lat'], 
				$value['centered_lng'], 
	...
	*/
	...
	function load_field( $field ) {
		...
		// set instruction
		$field['instructions'] = __( 'Use this map to preview the style, and to set your default zoom level. You may also drag and drop center location as needed.', 'acf-mapifylite' );
