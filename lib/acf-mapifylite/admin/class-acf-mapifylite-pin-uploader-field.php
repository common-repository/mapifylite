...
if( ! class_exists( 'Acf_Mapifylite_Pin_Uploader_Field' ) ) :


class Acf_Mapifylite_Pin_Uploader_Field extends acf_field {
	...
	function __construct( $settings ) {
		...
		$this->label = __( 'Mapify Pin Uploader', 'acf-mapifylite' );
		 ...
		 * var message = acf._e('mapify_pin_uploader', 'error');
		 */
	...
	function render_field( $field ) {		
		...
		$image_id      = isset( $value['image_id'] ) ? intval( $value['image_id'] ) : null;
		 ...
		 * Load admin fields
		 */
		include_once( 'partials/mapify-pin-uploader-field-display.php' );
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {	
		...
		$value['image_url'] = $mapify_map->get_pin_image_url();
	...
	function update_value( $value, $post_id, $field ) {
		...
		$image_id  = isset( $value['image_id'] ) ? intval( $value['image_id'] ) : 0;
		$image_url = isset( $value['image_url'] ) ? $value['image_url'] : null;
		...
		if ( isset( $value['image_url'] ) ) {			
			$mapify_map = new Mapify_Map( $post_id );
			$mapify_map->set_pin_image_url( $value['image_url'] );
		}
	...
	*/
	...
	function load_field( $field ) {
		...
		// set instruction
		$field['instructions'] = __( 'This image will serve as a default pinpoint for your map. Please save the map to apply the changes.', 'acf-mapifylite' );
